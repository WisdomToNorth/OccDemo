...
{
public:
    K_Context(const Handle(V3d_Viewer)& MainViewer) :AIS_InteractiveContext(MainViewer) {}
    void addToSelected(Handle(SelectMgr_EntityOwner)obj)
    {
        selected_.emplace_back(obj);
    }
    void clearSelectedVec() { selected_.clear(); }
    Handle(SelectMgr_EntityOwner) getSelected()
    {
        if (selected_.empty())return nullptr;
        else return selected_.front();
    }
private:
